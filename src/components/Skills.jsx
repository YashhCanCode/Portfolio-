  ...
  const SkillCard = ({ category, skills, index }) => {
    return (
      <motion.div
        ...
        className="group relative bg-white border border-black/5 rounded-3xl p-6 h-fit shadow-sm hover:shadow-xl hover:shadow-brand-accent/5 transition-all duration-500"
      >
        {/* Subtle glow on hover */}
        <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-brand-accent/0 via-brand-accent/0 to-brand-accent/0 group-hover:from-brand-accent/5 group-hover:to-brand-accent/5 transition-all duration-500 pointer-events-none" />
          ...
          <div className="flex flex-wrap gap-2">
            {skills.map((skill, idx) => (
              <motion.button
                ...
                initial="initial"
                whileHover="hover"
                className="px-3 py-1.5 text-xs font-medium text-black bg-black/3 hover:bg-brand-accent/10 border border-black/5 hover:border-brand-accent/20 rounded-full transition-all duration-300 cursor-default select-none hover:text-brand-medium"
              >
                {skill}
              </motion.button>
            ))}
  ...
  return (
    <section id="skills" className="relative w-full bg-white py-16 md:py-20 overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute top-10 right-10 w-32 h-32 bg-brand-accent rounded-full blur-3xl pointer-events-none"
      />
      ...
      <motion.div
        ...
        className="absolute bottom-20 left-5 w-40 h-40 bg-brand-accent rounded-full blur-3xl pointer-events-none"
      />
    </section>
  );
};
